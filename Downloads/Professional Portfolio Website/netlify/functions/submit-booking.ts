...
const DEFAULT_ADMIN_EMAIL = 'ibrahim.abdullayev1@gmail.com';
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Arial, sans-serif; background-color: #f1f5f9;">
  <div style="max-width: 600px; margin: 20px auto; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
    ...
    <div style="background: #f8fafc; padding: 20px; text-align: center; border-top: 1px solid #e2e8f0;">
      <p style="margin: 0; color: #64748b; font-size: 12px;">Bu email Portfolio Website-dən avtomatik göndərilmişdir.<br><strong>${DEFAULT_ADMIN_EMAIL}</strong></p>
    </div>
  </div>
</body>
</html>`;
...
async function sendEmail(data: ContactData): Promise<EmailResult> {
  ...
  if (!RESEND_API_KEY) {
    console.log('ℹ️ RESEND_API_KEY təyin edilməyib. Email göndərilməyəcək.');
  ...
  try {
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        warning: !email.success ? 'Email göndərilmədi. RESEND_API_KEY environment variable-ı təyin edilməyib.' : null,
      }),
    };
  } catch (error) {
